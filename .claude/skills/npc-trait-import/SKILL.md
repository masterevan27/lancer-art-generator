...
| `mil` | Role, Faction, Outfit, Headgear, Weapon, Gear | An issued uniform or military-issue equipment. **What it does differs by table, and assuming otherwise is the trap.** On Faction, Outfit and Headgear it is dropped for a civilian Role (and `civ` is dropped for a `mil` one) — that is `filter_by_mil()`, and Weapon and Gear are not in it. A civilian may carry military-issue kit; on those two tables `mil` is instead what a `notac` Outfit drops. |
...
| `unaffiliated` | Faction | The bullet is **not** an affiliation — the entries whose visual is empty because there is nothing to show. A marker rather than a preference, the same shape `none` has on Weapon and `bare` on Headgear: nothing is dropped *for* it, and it exists so `filter_by_affiliation()` can name the non-affiliations without matching on their prose. It is what a Role in `UNAFFILIATED_ROLES` ("a freelance salvager") has its whole Faction pool cut down to, since "freelance … Affiliation: House Clawthorne" is a dossier disagreeing with itself. Exactly two bullets carry it, and **never author one from a reference image** — an image cannot show an absence. |
...
| `blade` | Weapon | An edged weapon — a katana, a sword, a knife, a dagger. Read through `WEAPON_ROLES` in `generate-npc.py`, which names the flag a Role's armament **must** carry, and reads the opposite way round from the `admin` role lock: a lock says *this bullet is only for these Roles*, while this says *these Roles get only this bullet*. A lock protects an emblem from the wrong job; this protects a job from the wrong emblem. It does **not** reserve the blades — everyone else still rolls them freely. It exists because "a close-quarters blade specialist" is flagged `mil`, the `mil` tier restricts the pool to `sidearm` bullets, and no blade is a pistol: the two intersected to nothing, fell back to the whole table and handed the specialist a gun. So the `WEAPON_ROLES` restriction **outranks** the `sidearm` one rather than composing with it. Every `blade` bullet also carries `weapon`. |
...
| `crown` | Headgear | Something that sits **on top of** the skull without enclosing it — a wide brim, a tall or ceremonial hat, a cap of any kind, a rig clamped over the crown. It drops the `updo` Hair cuts, and nothing else; the filter runs both ways, so a pinned crowning hat drops the `updo` cuts instead. The softer half of a pair with `helmet`, which divides the head into three registers: `helmet` (the head is inside it) drops `updo` Hair **and** carried `helmet` Gear, `crown` drops `updo` Hair only, and an unflagged bullet drops nothing. That middle row is why this is not just `helmet` on the hats — a sedge hat has no quarrel with a flight helmet carried under one arm. Orthogonal to `hardtech`: that flag is a claim about *register* and this one about *volume*, so a cybernetic headpiece clamped over the crown honestly carries both — but a woven, straw or lacquered hat must **never** pick up `hardtech`, since `notac` drops it and those hats are what a kimono should reach. **Not** for anything that leaves the top of the head free: a brow visor, a headset, an earpiece, an ear implant, goggles pushed up on the forehead, or a hood, which has slack enough to go over gathered hair. |
...
| `outlaw` | Headgear | The Headgear **role lock**, the same mechanism as `admin`: reachable only by the criminal Roles named against it in `ROLE_LOCKS`, which is exactly the Criminals bucket: the pirate, smuggler, data pirate, data-rat courier, black-market broker and undercity fixer. Stage it only on headgear that openly declares piracy or crime — a skull-and-crossbones on a hat — not on anything a law-abiding spacer might also wear, such as a bandana or a tricorn with no pirate insignia. |
...
| `deskwork` | Backdrop | Occupation gate: operating a command, plot, watch or surveillance station. Pilots, Soldiers, Officials, Support, Criminals, Civilians, Scientists — every bucket but Laborers and Technicians, since a screen wall is genre-neutral in a way a cockpit is not. |
| `lab` | Outfit, Backdrop | Laboratory wear or a laboratory. **Differs by table.** On Outfit it is a *preference*: `ROLE_PREFERENCES` weights `lab` outfits up to 40% of a Scientist's pool, and anyone else may still roll one. On Backdrop it is also an occupation gate — only Scientists reach the scene — and the same preference weights it to 25% of a Scientist's scenes. Unlike every other gate it is a place, not an activity: tag a blurred laboratory too. |
...
| `ground` | Glow placement | Prop gate: the light pools on the floor, so the rolled Backdrop scene has to have the subject's weight on one. Matched on the *verb* the scene gives them — standing, walking, crouched — and matched in the scene's **opening clause only**, up to the first comma or dash. Both halves matter: a full-body scene opens `{Subject} {is_are} <verb>`, so the opening clause is the one span that is about the subject rather than the scenery, while a half-body scene describes only the background and carries no verb for the subject at all. Searching the whole sentence instead let "a hatch stands cycling open behind" keep the ground glow on a weightless tumble and "a work-mech kneeling in its cradle" keep it on a half-body hangar shot. `braced` is deliberately not one of the verbs — a subject can be braced in the open door of a hovering craft with the street far below. Pairs with `scene`. |
| `wall` | Glow placement | Prop gate: an interior surface behind the subject — a corridor, a bay, an alley, a room. Pairs with `scene`. |
| `screens` | Glow placement | Prop gate: a stacked display wall — monitors, readouts, consoles, a plot board. Pairs with `scene`. |
| `signage` | Glow placement | Prop gate: a lit skyline — signage, neon, billboards. Pairs with `scene`. |
...
| A garment, armor, or full kit | **Outfit** (or `Outfit (she) +` if the cut only reads on a woman's figure), **or the group table for its family** | Tag `civ`/`mil`, plus `notac` if it is elaborate or traditional, `dressy` if it is finery, and `lab` if it is laboratory wear such as a lab coat. The two are not the same — see the flag table. **Check the group headings first**: `Outfit` enters twenty-odd family tables through `- => Name` references (flight suits, work coveralls, kimonos and fine robes, glowing-seam bodysuits…), and a variant of one of those families goes under that group's exact heading, or its `(she) +` variant, not under `Outfit`, where it would weigh as much as a whole family. See §4's group note, and the group rule in §0's theme trap before tagging one. |
...
| A distinctive *placement* of the glow — rim light from behind, colour pooling on the ground, a haze hanging in the air | **Glow placement** | Portrait only. The predicate of "A faint {glow} glow ___", so it starts with a verb and never names the colour. Tag `scene` only if the light lands out in the environment rather than on the figure, and never let an unflagged one claim the whole shot — see §4. If the predicate names a **prop** — the ground, a wall, a display wall, a signage skyline, or air to hang in — add that prop gate (`ground`, `wall`, `screens`, `signage`, `air`) alongside `scene`, or the placement will land on scenes that have no such thing. A predicate that names something *worn* has no gate available and must be reworded to something every roll has ("{possessive} clothing", "the near shoulder"). |
...
- **Backdrop**: `<opening shot phrase> || <scene sentence> || [nogear] [weather] [cockpit] [ownmech] [mechwork] [mechyard] [warzone] [frontline] [vacuum] [swordwork] [deskwork] [ceremony] [inspection] [salvage] [clergy] [medic] [barkeep] [lab]` (may start with `xN `)
...
- **Weapon**: `<noun phrase, may use {possessive}> || [hands] [gun] [mil] [weapon] [simple] [sidearm] [blade]`
...
- **Gear**: `<noun phrase, may use {possessive}> || [hands] [mil] [helmet] [admin]`
...
- **Outfit**: `<noun phrase clause> || [civ] [mil] [notac] [dressy] [lab]`
...
- **Headgear**: `{Subject} {wear} <full sentence>. || [hardtech] [helmet] [crown] [mil] [outlaw]`
...
- **Faction**: `<name> || <visual, about a dozen words> || [civ] [mil] [palette] [dressy] [unaffiliated]`
...
- **Glow placement**: `<verb-initial predicate, may use {possessive}/{object}> || [scene] [ground] [wall] [screens] [signage] [air]`
