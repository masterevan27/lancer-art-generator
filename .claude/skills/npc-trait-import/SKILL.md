...
| `ground` | Glow placement | Prop gate: the light pools on the floor, so the rolled Backdrop scene has to have the subject's weight on one. Matched on the *verb* the scene gives them — standing, walking, crouched — which is what also drops the weightless scenes (they say floating and drifting) and every half-body one, since those describe only the background and carry no verb for the subject at all. Pairs with `scene`. |
