...
**Alternate look (reference sprite):** built from an actual reference sprite rather than the book's flavor text alone. Same generic frontline-soldier role as the version above, but a distinct silhouette — dark gunmetal-grey rather than olive-drab, teal-cyan optic rather than amber, and a heavier, wider-braced stance. Fits the standard front-facing token convention fine, so either version can be used as the table's "default" Assault look.

```
A full-body illustration of a heavy-set, wide-stanced humanoid combat mech, standing and facing directly forward, entire body visible from the top of its head to the base of its feet with clear empty space above and below, rendered in a detailed painterly illustration style matching worn military hardware: weathered dark gunmetal-grey armor plating with rust streaks, scuffed paint, and stenciled panel markings, fine grain texture, halftone dot shading in the shadows. Its boxy, low-slung head carries a single horizontal teal-cyan optic visor slit, set on thick, hunched shoulders with twin raised triangular sensor fins jutting up behind them, and a tall diamond-shaped backpack unit rising above. Its legs are thick and bent in a low, braced stance, feet flared outward and firmly planted. A large two-handed heavy rifle is gripped low and braced across its body with both arms. The mech stands fully grounded on both feet, planted and fully visible, in a low, ready stance. The background is a solid flat plain white, no texture, no gradient, no shadow, no environment. Centered composition, even lighting, isolated illustration, clean silhouette.
```
...
**Alternate look — heavy cavalry (shield & lance):** built from an actual reference sprite rather than the book's flavor text alone, and arguably a better fit for the name "Cataphract" (historically heavy armored cavalry fighting with lance and shield) than the Ram Cannon version above. Kept as a three-quarter dynamic pose rather than the front-facing/isolated-token format used everywhere else in this doc, since the angled shield-and-lance stance is the whole point of the design — convert it to a front-facing standing pose first if it needs to crop into a Foundry token like the other entries.

```
A three-quarter angle illustration of a heavy, wide-stanced combat mech built like armored cavalry, legs splayed low and bent, flared feet planted for a braced charge. Rendered in a detailed painterly illustration style matching worn military hardware: weathered dark gunmetal-grey armor plating with rust streaks, scuffed paint, and stenciled panel markings, fine grain texture, halftone dot shading in the shadows. Its boxy, angular head carries a single horizontal teal-cyan optic visor slit and a short sensor antenna, set atop blocky, oversized shoulder plating and a raised backpack unit. A massive round riveted shield, its face dented and battle-worn with a raised central boss, is braced on one arm. The opposite hand grips a long, tapering lance-like spike weapon extended forward, its point catching a faint cold glint. The mech is caught mid-charge, weight forward and leaning into the shield, in a dynamic three-quarter pose rather than a static standing one. The background is a solid flat plain white, no texture, no gradient, no shadow, no environment. Even lighting, isolated illustration, clean silhouette.
```
